...
import net.w2s.driverapp.service.ChatService;
...
public class MessageActivityNew extends AppCompatActivity implements ServiceCallbacks {
    ...
    private ChatReciver mChatReciver;
    private Utils utils;
...
//for arabic language.
        if (Utility.getSharedPreferences(MessageActivityNew.this, ConstantKeys.Setting_Language).equals("1")) {
            ViewCompat.setLayoutDirection(findViewById(R.id.message_root_view), ViewCompat.LAYOUT_DIRECTION_RTL);
        } else {
            ViewCompat.setLayoutDirection(findViewById(R.id.message_root_view), ViewCompat.LAYOUT_DIRECTION_LOCALE);
        }
    ...
    private void connectSocket() {
        try {
            ...
            ws = factory.createSocket(Utils.URL_WEBSOCKET + "" + Utility.getSharedPreferences(MessageActivityNew.this, ConstantKeys.USER_ID));
    ...
    private void sendChatMessage1() {
        ...
        if (Utility.isStringNullOrBlank(chatMsg) == false) {
            ...
            Toast.makeText(appContext, getString(R.string.type_msg), Toast.LENGTH_LONG).show();
        }
    }
    ...
    class SendMessageToAdmin extends AsyncTask<String, String, String> {
        ...
        protected String doInBackground(String... params) {
            String URL = ConstantKeys.SERVER_URL + "driver_send_message?sender_id=" + params[0] + "&reciever_id=" + params[1] + "&msg=" + params[2];
            ...
            try {
                ...
                if (networkResponse.equals(null) || networkResponse.equals("")) {
                    ...
                    if (networkResponse.getString(ConstantKeys.RESULT).equals("success")) {
                        ...
                        chatArrayAdapter.notifyDataSetChanged();
                        listView.setSelection(listView.getCount());*/
                        //Toast.makeText(appContext,"sent!",Toast.LENGTH_LONG).show();
                    } else {
    ...
    private class GetChatListTask extends AsyncTask<String, String, String> {
        ...
        @Override
        protected String doInBackground(String... params) {
            String URL = ConstantKeys.SERVER_URL + "driver_chatting?reciever_id=" + params[0];
            Log.e("URL", URL);
/*            String responce= Utility.findJSONFromUrl(URL);
            Log.e("GetChatListTask", "" + responce);
            return responce;*/
                        ...
                        if (networkResponse.has("details")) {
                            if (networkResponse.getJSONArray("details").length() > 0) {
                                ...
                                for (int i = 0; i < networkResponse.getJSONArray("details").length(); i++) {
                                    ...
                                    // chmsg.setSender(jsonObject.getString("user_name"));
                                    chatArrayList.add(chmsg);
                                    //chatArrayList.add(new ChatMessage(jsonObject.getString("sender"),jsonObject.getString("msg"),jsonObject.getString("user_name")));
                                }
                                chatArrayAdapter.notifyDataSetChanged();
    ...
    private void registerChatReceiver() {
        mChatReciver = new ChatReciver();
        registerReceiver(mChatReciver, new IntentFilter(ChatService.ChatTag));
    }

    private void unregisterChatReceiver() {
        unregisterReceiver(mChatReciver);
    }
                ...
                try {
                    ...
                    if (!jsonObject.optString("user_id", Utility.getSharedPreferences(MessageActivityNew.this, ConstantKeys.Reciever_ID)).equals(Utility.getSharedPreferences(MessageActivityNew.this, ConstantKeys.Reciever_ID))) {
                        ...
                        if (message.equals("1")) {// user is typing
                            ...
                            removeUnread();
                        } else { // there is new message    SimpleDateFormat simpleDateFormat1 = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
                            /* status=0 means unread and status=1 means read*/
                            ChatMessage chmsg = new ChatMessage();
                            chmsg.setSide("0");
